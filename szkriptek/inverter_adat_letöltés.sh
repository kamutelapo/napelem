...
curl -c $STOREDIR/cookie.txt https://www.aisweicloud.com/login -o $STOREDIR/Login.html


curl -b $STOREDIR/cookie.txt -c $STOREDIR/cookie.txt -X POST  -d "username=$USER" -d "password=$PASSWORD" https://www.aisweicloud.com/login -o /dev/null

curl -b $STOREDIR/cookie.txt -c $STOREDIR/cookie.txt https://www.aisweicloud.com/station -o $STOREDIR/Station.html
...
curl -b $STOREDIR/cookie.txt -c $STOREDIR/cookie.txt https://www.aisweicloud.com/config/equip/deviceList?stationId=$STATION_ID -o $STOREDIR/DeviceList.json
...
do
  ...
  curl -b $STOREDIR/cookie.txt -c $STOREDIR/cookie.txt -X POST -d "downloadSid=$STATION_ID" -d "deviceList=$DEVICE_ID" -d "dataClassification=inv_data" -d "search_EQ_sdate=$DOWNLOAD_DATE" -d "search_EQ_edate=$DOWNLOAD_DATE" -d "selectFileds=Pac" -d "selectFileds=Fac" -d "selectFileds=Temp" -d "selectFileds=E-Today" -d "selectFileds=E-Total" -d "selectFileds=H-Total" -d "selectFileds=Ipv1" -d "selectFileds=Ipv2" -d "selectFileds=Ipv3" -d "selectFileds=Vpv1" -d "selectFileds=Vpv2" -d "selectFileds=Vpv3" -d "selectFileds=Vac1" -d "selectFileds=Vac2" -d "selectFileds=Vac3" -d "selectFileds=Iac1" -d "selectFileds=Iac2" -d "selectFileds=Iac3" https://www.aisweicloud.com/management/inv/downloadAll/$DEVICE_ID -o $STOREDIR/$DATED_FILENAME.xls
